...
public class NonBalancingBST 
{
    protected string Root { get; set; }  //Initially hard-wired to string type

    protected NonBalancingBST  LeftChild { get; set; }

    protected NonBalancingBST  RightChild { get; set; }

    public int Depth { get; protected set; }

    public NonBalancingBST(string root) //Constructor
    {
        Root = root;
        Depth = 1;
    }

    public NonBalancingBST(string root, NonBalancingBST  left, NonBalancingBST  right) : this(root)
    {
        LeftChild = left;
        RightChild = right;
        CalculateDepth();
    ...
    private void AddToLeftChild(string newValue)
    {
        ...
        {
            LeftChild = new NonBalancingBST (newValue);
    ...
    private void AddToRightChild(string newValue)
    {
        ...
        {
            RightChild = new NonBalancingBST (newValue);
    ...
    public List<string> TraverseBreadthFirst()
    {
        ...
        var queue = new Queue<NonBalancingBST >();
        queue.Enqueue(this);
        NonBalancingBST  visiting = null;
        while (queue.Count > 0)
        {
    ...
    private void PushAllChildrenOntoStack(Stack<string> stack, NonBalancingBST  tree)
    {
        if (tree != null)
        {
